#!/usr/bin/env python3
"""Resume mamba_ablation from experiment 3 (LeakyTernaryLIF). Exps 1-2 done."""
...
# --- Top-K LIF (same as mamba_ablation.py) ---
class TopKSparseTernaryLIF(nn.Module):
    def __init__(self, n_neurons, beta=0.9, base_threshold=1.5, soft_reset=True, top_k_frac=0.3):
        super().__init__()
        self.beta = beta; self.base_threshold = base_threshold
        self.soft_reset = soft_reset; self.top_k_frac = top_k_frac
    ...
    def forward(self, x, mem=None):
        ...
        pos_fire = mem > threshold; neg_fire = mem < -threshold
        B, T, D = x.shape
        k = max(1, int(D * self.top_k_frac))
        _, topk_idx = mem.abs().topk(k, dim=-1)
        topk_mask = torch.zeros_like(mem, dtype=torch.bool)
        topk_mask.scatter_(-1, topk_idx, True)
        pos_fire = pos_fire & topk_mask; neg_fire = neg_fire & topk_mask
        spikes = torch.zeros_like(mem)
        spikes[pos_fire] = 1.0; spikes[neg_fire] = -1.0
        if self.soft_reset: mem = mem * (1.0 - spikes.abs())
        else: mem = mem - spikes * threshold
        with torch.no_grad(): self.threshold_trace = spikes.abs().mean(dim=(0, 1))
        return spikes, mem

class TopKLeakyTernaryLIF(nn.Module):
    def __init__(self, n_neurons, beta=0.9, base_threshold=1.5, soft_reset=True, init_alpha=0.95, top_k_frac=0.3):
        super().__init__()
        self.lif = TopKSparseTernaryLIF(n_neurons, beta, base_threshold, soft_reset, top_k_frac)
        self.alpha_logit = nn.Parameter(torch.tensor(math.log(init_alpha / (1 - init_alpha))))
        self.register_buffer("threshold_trace", torch.zeros(n_neurons))
        self.threshold_offset = self.lif.threshold_offset
    @property
    def alpha(self): return torch.sigmoid(self.alpha_logit)
    @property
    def effective_threshold(self): return self.lif.effective_threshold
    def forward(self, x, mem=None):
        spikes, mem = self.lif(x, mem)
        a = self.alpha
        out = a * spikes + (1 - a) * F.silu(x)
        with torch.no_grad(): self.threshold_trace = self.lif.threshold_trace
        return out, mem
...
def make_mamba(neuron_mode="leaky", top_k_frac=None):
    config = SpikeMambaConfig(
        ...
        ternary=(neuron_mode != "binary"), ternary_threshold=1.5,
        leaky_ternary=(neuron_mode == "leaky"), leaky_alpha_init=0.95,
        continuous_gate=True, soft_reset=True,
        adaptive_threshold=(neuron_mode == "binary"), surrogate="cauchy",
    )
    ...
    if top_k_frac is not None:
        for block in model.blocks:
            block.lif_out = TopKLeakyTernaryLIF(
                ...
                init_alpha=0.95, top_k_frac=top_k_frac).to(device)
    return model
...
def train_kd(model, name, steps=STEPS, use_twn=False):
    ...
    for step in range(1, steps + 1):
        ...
        if step % 500 == 0:
            ppl = eval_ppl(model); fire_rate = measure_sparsity(model)
            alphas = get_alphas(model); best_ppl = min(best_ppl, ppl)
    ...
    final_ppl = eval_ppl(model); final_fr = measure_sparsity(model)
    return {
        'name': name, 'final_ppl': round(final_ppl, 1), 'best_ppl': round(best_ppl, 1),
        'final_fire_rate': final_fr, 'final_sparsity': round(1.0 - final_fr, 4),
        'final_alphas': [round(a, 3) for a in get_alphas(model)],
        'n_params': sum(p.numel() for p in model.parameters()), 'history': history,
    }
...
with open(out_path) as f:
    results = json.load(f)
print(f"Loaded {len(results)} existing results: {list(results.keys())}\n", flush=True)
...
for name, r in sorted(results.items()):
    ...
    print(f"  {name:35s} | PPL {r['best_ppl']:7.1f} | Δ {delta:+6.1f}% | fire={fr:.1%} | [{a_str}]")
print(flush=True)
